...
- 백엔드 데이터가 없으면 고정 목업을 만들지 않고 `응답 없음` 또는 입력 필요 상태로 표시합니다.
- BE2 계약 응답에는 금액/기간 필드가 없기 때문에, 프론트에서 사용자가 입력한 계약 조건을 BE1 에스크로 생성 요청에 함께 사용합니다.
- 임차인/임대인 지갑 주소가 모두 준비되지 않으면 계약 생성과 에스크로 생성 버튼이 비활성화됩니다.
- 자동 반환 화면은 계약 만료일 + 7일을 예상 반환일로 계산합니다.
- 평판 단계는 아직 실제 점수 API가 없어 등급을 계산하지 않고 대기 상태로 표시합니다.
